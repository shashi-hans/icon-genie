...
export default handler(async (req, res) => {
  ...
  // Checked so a typo returns 404 instead of silently filing a hidden name that
  // matches nothing. hasIcon answers from the cached seed for a built icon and
  // with one indexed lookup otherwise, which matters because this endpoint gets
  // pressed repeatedly while sifting duplicates.
  if (!(await store.hasIcon(name))) throw new HttpError(404, `No icon named "${name}".`);
